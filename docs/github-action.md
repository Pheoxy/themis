...
- `format`: Themis output format. Use `markdown`, `comment`, `json`, or `sarif`. `config-check` supports only `markdown` and `json`. Default: `markdown`.
...
The `config-check` workflow ignores gate-only inputs such as `base`, `body-file`, `evidence`, and `annotations`; it rejects output formats other than `markdown` and `json`.
