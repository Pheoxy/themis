...
Themis should act like an assistant in the goddess's domains: law, order, custom, and proper procedure. It should help contributors discover rules, organize evidence, prepare pull request text, run checks, and draft PRs when the gate passes. It should also help reviewers and maintainers by turning repeat process failures into clear contributor-facing feedback, so maintainers can spend less time restating rules and more time reviewing project-specific substance. Those assistant workflows still serve the pass/fail gate. The assistant role supports the contributor and maintainer; it does not replace their responsibility.
